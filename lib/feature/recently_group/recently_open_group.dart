...
final List<RecentlyOpenGroup> dummyRecentlyOpenGroups = [
  RecentlyOpenGroup(
    title: '거북 베이킹',
    location: '수원',
    price: 30000,
...
// --- 3. 화면을 그리는 메인 위젯 ---
class RecentlyOpenGroupScreen extends StatefulWidget {
  final List<RecentlyOpenGroup> groups;
  const RecentlyOpenGroupScreen({super.key, required this.groups});

  @override
  State<RecentlyOpenGroupScreen> createState() => _RecentlyOpenGroupScreenState();
}

class _RecentlyOpenGroupScreenState extends State<RecentlyOpenGroupScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          _buildRecentlyOpenGroupAppBar(context),
  ...
  // 최근 개설된 모임 AppBar(뒤로가기+타이틀)를 위젯으로 분리
  Widget _buildRecentlyOpenGroupAppBar(BuildContext context) {
    return Container(
      color: Colors.white,
      child: SafeArea(
        bottom: false,
  ...
  // 각 모임 목록 아이템을 만드는 위젯
  Widget _buildGroupListItem(RecentlyOpenGroup group) {
    return Card(
...
void main() {
  runApp(
    MaterialApp(
      ...
      home: RecentlyOpenGroupScreen(groups: dummyRecentlyOpenGroups),
    ),
  );
}